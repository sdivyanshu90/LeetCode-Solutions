...
- **Time Complexity**: - Let n = current number of elements in stream. - Binary search: O(log n). - Insertion: O(n) due to array shift. - Per add() call: O(n). - Constructor: O(n log n) for initial sort. Space complexity - Storing all stream elements: O(n). - Space complexity: O(n). Thought process and trade-offs - Sorted list approach: simple but not optimal for large streams. - Alternative: use min-heap of size k - O(log k) per add, O(k) space, more efficient for large streams. - Min-heap keeps k largest elements; heap top is kth largest. - Current approach: easier to understand but less efficient. - For small k or small stream size, difference is negligible. - For production: heap-based solution is preferred.
- **Space Complexity**: - Storing all stream elements: O(n). - Space complexity: O(n). Thought process and trade-offs - Sorted list approach: simple but not optimal for large streams. - Alternative: use min-heap of size k - O(log k) per add, O(k) space, more efficient for large streams. - Min-heap keeps k largest elements; heap top is kth largest. - Current approach: easier to understand but less efficient. - For small k or small stream size, difference is negligible. - For production: heap-based solution is preferred.
