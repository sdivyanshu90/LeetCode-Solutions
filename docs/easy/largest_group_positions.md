...
- **Time Complexity**: - Let n = length of string. - Single pass through string: O(n). - Each position processed once. - Overall time complexity: O(n). Space complexity - Result list stores at most k groups where k <= n/3. - Space complexity: O(k) = O(n) worst case. Thought process and trade-offs - Two-pointer approach: efficient single-pass solution. - Combined condition `j == len(s) - 1 or s[j] != s[j + 1]` handles both end of string and character change elegantly. - Alternative: separate loops for each group - more complex logic. - Current approach: clean and optimal. - No preprocessing needed: direct string traversal.
- **Space Complexity**: - Result list stores at most k groups where k <= n/3. - Space complexity: O(k) = O(n) worst case. Thought process and trade-offs - Two-pointer approach: efficient single-pass solution. - Combined condition `j == len(s) - 1 or s[j] != s[j + 1]` handles both end of string and character change elegantly. - Alternative: separate loops for each group - more complex logic. - Current approach: clean and optimal. - No preprocessing needed: direct string traversal.
