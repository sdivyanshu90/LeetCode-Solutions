...
You are given an `n x n` binary matrix `grid`. You are allowed to change <b>at most one</b> `0` to be `1`.

Return the size of the largest <b>island</b> in `grid` after applying this operation.
