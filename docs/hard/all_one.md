...
Design a data structure to track the frequency of strings and get the string with maximum/minimum frequency in O(1) time.

**LeetCode Problem**: [View on LeetCode](https://leetcode.com/problems/all-oone-data-structure/)

## Approach: Doubly Linked List with Hash Map (Implemented)

### Strategy

Maintain a doubly linked list of frequency nodes. Each node contains a set of strings with that frequency. Use hash map to map strings to their frequency nodes.

Use a doubly-linked list where nodes represent frequencies. When incrementing a key's frequency, move it from current frequency node to the next one. Maintain hash map for O(1) access.
...
2. **Main Algorithm**: Execute the core doubly linked list with hash map logic
...
- Input: `inc('leet'); getMaxKey()`
- Expected: `'hello'`
- The algorithm processes the input step by step

**Example 3**:

- Input: `dec('hello'); getMaxKey()`
- Expected: `'leet'`
- The algorithm processes the input step by step

### Why Doubly Linked List with Hash Map Works

This approach is optimal because:

1. O(1) operations for all methods
2. Efficient frequency tracking
3. Quick max/min retrieval
...
- **Space Complexity**: O(n) where n is total number of unique strings
...
- Quick max/min retrieval

### Disadvantages

- Complex implementation with linked list manipulation
- Need to handle node creation and deletion carefully
- More memory overhead compared to simple hash map
...
| ---------------------------------------------- | --------------------------------------------------- | ---------------------------------------------- | ------------- |
| Doubly Linked List with Hash Map (Implemented) | O(1) for inc(), dec(), getMaxKey(), and getMinKey() | O(n) where n is total number of unique strings | Hard          |
...
1. **Algorithm Selection**: Choosing doubly linked list with hash map for efficiency
...
- Related problems using doubly linked list with hash map
...
**Difficulty**: Hard  
**Topics**: Doubly Linked List with Hash Map  
